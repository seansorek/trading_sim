...
| **final eval (2026-07-15)** | **0.0407** | **0.228** | **0.776** | PBO rose from 0.067 (gate) to 0.228 because the `bcd149b` clip-at-fit_transform fix triggered a fresh `train_predictor.py` run which found a different optimal (q=0.75, w=40) vs the gate run's params; still well below v3 baseline of 0.514 |
