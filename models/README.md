...
| **Feature set** | `daily_v3` — 30 normalized daily technical features (see `daily_features.FEATURE_COLS`) |
...
| **Feature set** | `daily_v3` — same 30 features |
...
| **State** | Rolling 20-day window × 30 features (flattened to 600-dim vector) |
...
- **Features**: 30 daily features defined in `daily_features.FEATURE_COLS`
...
- **Algorithm**: `xgboost.XGBClassifier` (`multi:softprob`, 3-class)
- **Features**: Same 30 features as logistic
...
30-feature single-bar technical-indicator set — `daily_hybrid`'s transformer component shows
...
2. **Bad architecture (action-classification vs. prediction+strategy split)?** Tested via
   ...
   (same purged split, same 30 features) recovers a small but real out-of-sample signal —
...
- **State**: Rolling window of last 20 days × 30 features (flattened to 600-dim vector)
