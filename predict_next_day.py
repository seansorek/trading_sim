...
def format_position_size(position_data: Dict[str, float]) -> str:
    ...
    # Calculate account size from dollar_amount and fraction, then apply conservative fraction
    account_size = position_data.get('dollar_amount', 100.0) / position_data['fraction'] if position_data['fraction'] > 0 else 100.0
    conservative_dollar = conservative * account_size
...
def send_webhook_message(message: str):
    """Send message to Discord webhook via WEBHOOK_URL secret."""
    ...
    # Discord webhooks use 'content', not 'text'
    # Also split long messages (Discord limit is 2000 chars per message)
    ...
    if len(message) <= max_length:
        ...
        # Split into multiple messages for long content
        lines = message.split('\n')
        current_msg = []
        current_length = 0
